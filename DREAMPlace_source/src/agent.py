...
from model.preference_predictor import PreferencePredictor
...
class PPOAgent():
    def __init__(self, args):
        ...
        # CarePlace preference predictor (shared between actor and env)
        self.preference_predictor = PreferencePredictor(args=args).to(args.device)

        # Actor includes preference predictor for gradient flow during PPO updates
        self.actor = Actor(args=args, cnn=self.cnn, cnn_coarse=self.cnn_coarse,
                           preference_predictor=self.preference_predictor).float().to(args.device)
        self.critic = Critic(args=args, cnn_coarse=self.cnn_coarse).float().to(args.device)

        # Single optimizer for actor + preference predictor (shared parameters)
        ...
        th.save(
            {
                ...
                'preference_predictor': self.preference_predictor.state_dict(),
            },
    ...
    def load_model(self, checkpoint_path):
        ...
        if 'preference_predictor' in checkpoint:
            self.preference_predictor.load_state_dict(checkpoint['preference_predictor'])

    def train(self):
        self.training = True
        self.actor.train()
        self.critic.train()
        self.preference_predictor.train()

    def eval(self):
        self.training = False
        self.actor.eval()
        self.critic.eval()
        self.preference_predictor.eval()
