...
from model.preference_predictor import MacroInteractionGraph
...
class ProblemInstance():
    ...
    def build_interaction_graph(self):
        """Build macro interaction graph for GNN-based preference prediction."""
        if not hasattr(self, '_interaction_graph'):
            self._interaction_graph = MacroInteractionGraph(
                node_info=self.node_info,
                node_to_net_dict=self.node_to_net_dict,
                net_info=self.net_info,
                macro_list=list(self.macro_pos.keys()),
            )
    ...
    def read_macro_pos_from_def(self, def_path):
        ...
        for macro in self.macro_pos:
            raw_x = float(database["nodes"][macro]['x']) * scaling_ratio_x
            raw_y = float(database["nodes"][macro]['y']) * scaling_ratio_y
