...
from model.preference_predictor import DynamicGroupCalibration
...
class PlaceEnv(gym.Env):
    def __init__(self, args) -> None:
        ...
        # State: 1 (place_idx) + 8*G*G (masks) + 2 (size) + 2 (preference) + 1 (problem_id)
        self.observation_space = spaces.Discrete(6 + 8 * self.grid * self.grid)
        self.placedb = args.placedb
        self.problem_train = self.placedb.problem_train
        self.problem_eval = self.placedb.problem_eval

        # Dynamic group calibration (CarePlace Eq. 10)
        self.calibration = DynamicGroupCalibration(
            epsilon=getattr(args, 'calibration_epsilon', 1e-8)
        )

        # Legacy reward scaling (replaced by dynamic calibration)
        self.wire_reward_max = -INF
        self.wire_reward_min = INF
        self.structural_reward_max = -INF
        self.structural_reward_min = INF
        ...
        # Preference prediction cache
        self.preference_predictor = None
        self._graph_cache_entry = None
        self._current_alpha_w = 0.5
        self._current_alpha_s = 0.5

        # Annealing state
        self._anneal_episode = 0
        ...
        for problem in set(self.problem_train + self.problem_eval):
            if self.args.dataset_path is not None and os.path.exists(self.args.dataset_path):
                ...
                shutil.copy2(os.path.join(self.dmp_temp_placement_path, problem.benchmark, f'{min_idx}.def'),
                            os.path.join(self.reference_placement_path, problem.benchmark, f'{problem.benchmark}_{min_hpwl/1e7:.4f}.def'))
                shutil.copy2(os.path.join(self.dmp_temp_placement_path, problem.benchmark, f'{min_idx}.png'),
                            os.path.join(self.reference_placement_path, problem.benchmark, f'{problem.benchmark}_{min_hpwl/1e7:.4f}.png'))
    ...
    def set_preference_predictor(self, predictor, episode=0):
        """Attach the preference predictor for perception-guided feedback."""
        self.preference_predictor = predictor
        self._anneal_episode = episode

    def set_anneal_episode(self, episode):
        self._anneal_episode = episode

    def _get_anneal_lambda(self):
        """Compute annealing factor lambda for preference guidance."""
        start = getattr(self.args, 'preference_anneal_start', 0.0)
        end = getattr(self.args, 'preference_anneal_end', 1.0)
        episodes = getattr(self.args, 'preference_anneal_episodes', 500)
        progress = min(1.0, self._anneal_episode / max(1, episodes))
    ...
    def reset(self, test_mode, benchmark=None, reward_scaling_flag=False):
        ...
        # Build GNN graph cache for this problem
        self._update_graph_cache()

        # Get masks for first macro
        _, _, size_x, size_y = self.old_macro_pos[self.macro_to_place[0]]
        structural_mask = self.get_structural_mask(self.macro_to_place[0])
        ...
        if len(self.macro_to_place) > 1:
            ...
            next_structural_mask = self.get_structural_mask(self.macro_to_place[1])
            ...
            next_structural_mask = np.zeros((self.grid, self.grid))
            next_position_mask = np.zeros((self.grid, self.grid))
            next_wire_mask = np.zeros((self.grid, self.grid))

        # Predict macro preference using CarePlace preference predictor
        # View mask = new_canvas (shows already-placed macros; empty for first macro)
        alpha_w, alpha_s = self._predict_preference(
            self.macro_to_place[0], new_canvas, position_mask, wire_mask, structural_mask
        )
        self._current_alpha_w = alpha_w
        self._current_alpha_s = alpha_s

        # Mask normalization
        structural_mask, next_structural_mask = self.__mask_normalization(structural_mask, next_structural_mask)
        wire_mask, next_wire_mask = self.__mask_normalization(wire_mask, next_wire_mask)

        problem_id = self.state_parsing.PROBLEM_IDS.get(self.problem.benchmark, 0)
        ...
        self.state = self.state_parsing.get_state(
            ...
            wire_mask=wire_mask,
            next_structural_mask=next_structural_mask,
    ...
    def _update_graph_cache(self):
        """Ensure graph cache entry is available for the current problem."""
        if self.preference_predictor is None:
            self._graph_cache_entry = None
            return
        self._graph_cache_entry = self.preference_predictor.build_graph_for_problem(self.problem)

    def _predict_preference(self, macro_name, view_mask, position_mask, wire_mask, structural_mask):
        """Predict macro preference [alpha_w, alpha_s] using CarePlace preference predictor."""
        if self.preference_predictor is None or self._graph_cache_entry is None:
            return 0.5, 0.5   # fallback: equal preference

        import torch as th
        device = next(self.preference_predictor.parameters()).device
        vm = th.from_numpy(view_mask).float().to(device)
        pm = th.from_numpy(position_mask).float().to(device)
        wm = th.from_numpy(wire_mask).float().to(device)
        sm = th.from_numpy(structural_mask).float().to(device)

        alpha = self.preference_predictor(
            macro_name, vm, pm, wm, sm, self._graph_cache_entry
        )
        return alpha[0].item(), alpha[1].item()
    ...
    def step(self, action):
        ...
        assert abs(size_x - self.state[-5] * self.grid) < 1e-5
        assert abs(size_y - self.state[-4] * self.grid) < 1e-5
        ...
        # Compute wirelength reward (CarePlace Eq. 7)
        # c_w(a) = (HPWL(P_ref ∪ a) - HPWL(P_ref)) / (|HPWL(P_ref)| + ε)
        wire_reward_mask = self.get_wire_mask(macro)
        raw_wire = -wire_reward_mask[x, y]
        ref_hpwl = max(abs(getattr(self.problem, 'mp_hpwl', 1.0)), 1.0)
        wire_reward = raw_wire / (ref_hpwl + 1e-8)
        for net_name in self.problem.node_to_net_dict[macro]:
        ...
        # Compute structural reward (CarePlace Eq. 8)
        # c_s(a) = S(a) / (G/2 + ε)
        raw_structural = -self.get_structural_mask(macro)[x, y]
        structural_reward = raw_structural / (self.grid / 2.0 + 1e-8)

        # Dynamic group calibration (CarePlace Eq. 10)
        self.calibration.update(wire_reward, structural_reward)

        alpha_w, alpha_s = self._current_alpha_w, self._current_alpha_s

        if self.args.use_reward_scaling and self.reward_scaling_flag:
            # Legacy reward scaling
            self.wire_reward_max = max(self.wire_reward_max, wire_reward)
            self.wire_reward_min = min(self.wire_reward_min, wire_reward)
            self.structural_reward_max = max(self.structural_reward_max, structural_reward)
            self.structural_reward_min = min(self.structural_reward_min, structural_reward)

        if self.args.use_reward_scaling:
            wire_reward_scaled = (wire_reward - self.wire_reward_min + 1e-10) / \
                (self.wire_reward_max - self.wire_reward_min + 1e-10)
            structural_reward_scaled = (structural_reward - self.structural_reward_min + 1e-10) / \
                (self.structural_reward_max - self.structural_reward_min + 1e-10)
        else:
            # Use dynamic group calibration
            wire_reward_scaled = self.calibration.normalize_wire(wire_reward)
            structural_reward_scaled = self.calibration.normalize_structural(structural_reward)

        # Perception-guided feedback adjustment (CarePlace Section 4.3)
        use_perception = getattr(self.args, 'use_perception_guidance', False)
        if use_perception and not self.reward_scaling_flag:
            anneal_lambda = self._get_anneal_lambda()
            beta_0 = self.args.wire_coeff  # static initial coefficient
            beta = (1.0 - anneal_lambda) * beta_0 + anneal_lambda * alpha_w
        else:
            beta = self.args.wire_coeff

        reward += legal_reward + beta * wire_reward_scaled + (1.0 - beta) * structural_reward_scaled

        # Advance to next macro
        self.place_idx += 1

        # Get masks for next macro(s)
        size_x, size_y = 0, 0
        structural_mask = np.zeros((self.grid, self.grid))
        position_mask = np.zeros((self.grid, self.grid))
        wire_mask = np.zeros((self.grid, self.grid))
        next_structural_mask = np.zeros((self.grid, self.grid))
        next_position_mask = np.zeros((self.grid, self.grid))
        next_wire_mask = np.zeros((self.grid, self.grid))
        next_alpha_w, next_alpha_s = 0.5, 0.5

        if self.place_idx < len(self.macro_to_place):
            _, _, size_x, size_y = self.old_macro_pos[self.macro_to_place[self.place_idx]]
            structural_mask = self.get_structural_mask(self.macro_to_place[self.place_idx])
            position_mask = self.get_position_mask(size_x=size_x, size_y=size_y)
            wire_mask = self.get_wire_mask(self.macro_to_place[self.place_idx])

            # Predict preference for next macro
            next_alpha_w, next_alpha_s = self._predict_preference(
                self.macro_to_place[self.place_idx], new_canvas,
                position_mask, wire_mask, structural_mask
            )
        ...
        if self.place_idx < len(self.macro_to_place) - 1:
            ...
            next_structural_mask = self.get_structural_mask(self.macro_to_place[self.place_idx + 1])
            next_position_mask = self.get_position_mask(size_x=next_size_x, size_y=next_size_y)
            next_wire_mask = self.get_wire_mask(self.macro_to_place[self.place_idx + 1])

        self._current_alpha_w = next_alpha_w
        self._current_alpha_s = next_alpha_s
        ...
        if self.place_idx >= len(self.macro_to_place):
            ...
            if self.test_mode and not self.reward_scaling_flag:
                ...
                os.makedirs(os.path.join(self.full_figure_path, f"{self.args.name}/{self.args.unique_token}/{self.problem.benchmark}"),
                            exist_ok=True)
                shutil.copy2(os.path.join(self.dmp_temp_placement_path, self.problem.benchmark, f"{min_call_id}.def"),
                            os.path.join(self.dmp_result_path, f"{self.args.name}/{self.args.unique_token}/{self.problem.benchmark}/{self.args.i_episode}_{self.args.t_env}_{min_global_hpwl/1e7:.4f}.def"))
                shutil.copy2(os.path.join(self.dmp_temp_placement_path, self.problem.benchmark, f"{min_call_id}.png"),
                            os.path.join(self.full_figure_path, f"{self.args.name}/{self.args.unique_token}/{self.problem.benchmark}/{self.args.i_episode}_{self.args.t_env}_{min_global_hpwl/1e7:.4f}.png"))
        ...
        info = {
            ...
            "wire_reward": wire_reward_scaled,
            "structural_reward": structural_reward_scaled,
            "wire_reward/structural_reward": np.abs(wire_reward_scaled) / (np.abs(structural_reward_scaled) + 1e-10),
    ...
    def get_structural_mask(self, macro):
        """CarePlace structural mask (Eq. 2): distance-to-boundary prior.

        S(i,j) = min{i, G-(i+w_k)} + min{j, G-(j+h_k)}

        This replaces the old regular_mask. It measures proximity to chip boundary,
        encouraging macros to stay away from edges when structurally appropriate.
        """
        _, _, size_x, size_y = self.old_macro_pos[macro]
        G = self.grid
        ...
        for i in range(G):
            for j in range(G):
                mask[i, j] = min(i, G - (i + size_x)) + min(j, G - (j + size_y))

        # Center at current position like the original
        x, y, _, _ = self.old_macro_pos[macro]
        mask -= mask[x, y]
    ...
    def get_wire_mask(self, macro):
        ...
        for net_name in self.problem.node_to_net_dict[macro]:
            if net_name in self.net_to_macro:
                ...
                if 'weight' not in self.problem.net_info[net_name]:
                    weight = 1.0
                else:
    ...
    def __mask_normalization(self, mask1, mask2):
        if np.abs(mask1).max() > 0 or np.abs(mask2).max() > 0:
            max_val = max(np.abs(mask1).max(), np.abs(mask2).max())
            mask1 /= max_val
            mask2 /= max_val
        return mask1, mask2
