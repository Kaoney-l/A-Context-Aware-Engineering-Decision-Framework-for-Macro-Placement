...
def process_args():
    ...
    for arg in params:
        key, value = arg.split('=')
        try:
            cmd_config_dict[key] = ast.literal_eval(value)
        except:
    ...
    print(f"CarePlace: train on benchmark:\t{args.benchmark_train}")
    print(f"CarePlace: eval on benchmark:\t{args.benchmark_eval}")
...
def main():
    ...
    # Wire up the preference predictor with the environment
    env.set_preference_predictor(agent.preference_predictor, episode=0)
    ...
    for i_episode in range(1, args.episode + 1):
        ...
        # Update environment with current episode for preference annealing
        env.set_anneal_episode(i_episode)
        ...
        if i_episode - last_test_episode >= args.test_interval:
            ...
            for benchmark in args.benchmark_eval:
                ...
                logger.add(f'reward/{benchmark}/structural_reward', reward_info['structural_reward'], args.t_env)
                logger.add(f'reward/{benchmark}/legal_reward', reward_info['legal_reward'], args.t_env)
                logger.add(f'reward/{benchmark}/wire_structural_reward_ratio',
                           np.mean(reward_info['wire_structural_reward_ratio']), args.t_env)

                if 'alpha_w' in reward_info:
                    logger.add(f'preference/{benchmark}/alpha_w', reward_info['alpha_w'], args.t_env)
                    logger.add(f'preference/{benchmark}/alpha_s', reward_info['alpha_s'], args.t_env)
                    logger.add(f'preference/{benchmark}/beta', reward_info.get('beta', 0), args.t_env)
...
def run(env, agent, t_env, test_mode=False, benchmark=None, reward_scaling_flag=False):
    ...
    while not done:
        ...
        reward_info['structural_reward'] += info['structural_reward']
        reward_info['legal_reward'] += info['legal_reward']
        reward_info['wire_structural_reward_ratio'].append(info['wire_reward/structural_reward'])
        reward_info['global_hpwl'] = 0 if not done else info['global_hpwl']
        reward_info['alpha_w'] = info.get('alpha_w', 0.5)
        reward_info['alpha_s'] = info.get('alpha_s', 0.5)
        reward_info['beta'] = info.get('beta', 0.7)
        if not test_mode:
