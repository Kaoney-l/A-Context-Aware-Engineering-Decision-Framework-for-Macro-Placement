...
def read_benchmark_from_def(database, benchmark, args=None):
    ...
    for id, macro in enumerate(macro_lst):
        # scaling
        place_x = float(database["nodes"][macro]['x']) * ratio_x
        place_y = float(database["nodes"][macro]['y']) * ratio_y
    ...
    with open(lef_file, "r") as f:
        for line in f:
            ...
            if "macro_name_or_pin_name" in output.keys():
                ...
                database["macro_size"][macro_name] = (float(output['size_x']), float(output['size_y']))
                ...
                if pin_flag:
                    x1, y1 = float(output["x1"]), float(output["y1"])
                    x2, y2 = float(output["x2"]), float(output["y2"])
                    pin_x.append(x1)
                    pin_x.append(x2)
                    pin_y.append(y1)
                    pin_y.append(y2)
    ...
    with open(def_file, "r") as f:
        for line in f:
            ...
            if "num_comps" in output.keys():
                ...
                database["design_config"][output["key"]] = float(output["value"])

            elif "lower_x" in output.keys():
                database["diearea_rect"].extend([float(output['lower_x']),
                                                float(output['lower_y']),
                                                float(output['upper_x']),
                                                float(output['upper_y'])])
